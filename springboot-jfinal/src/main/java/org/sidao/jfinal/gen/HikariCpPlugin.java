...
public class HikariCpPlugin implements IPlugin, IDataSourceProvider{
	...
	private boolean hasProxyDriver;

	private String proxyDriverClass;
	/**
	 * username
	 */
	...
	/**
	 * @param configFile 扩展配置文件
	 * @param hikariConfigFile hikari.properties
	 * @param hasProxyDriver 是否通过代理驱动
	 */
	public HikariCpPlugin(String configFile,String hikariConfigFile, boolean hasProxyDriver) {
		this.hikariConfigFile=hikariConfigFile;
		this.configFile=configFile;
		this.hasProxyDriver=hasProxyDriver;
	}
	...
	public HikariCpPlugin(String jdbcUrl, String username, String password, String proxyDriverClass,String driverClass) {
		this.jdbcUrl = jdbcUrl;
		this.username = username;
		this.password = password;
		this.driverClass = driverClass;
		this.proxyDriverClass=proxyDriverClass;
	}
	...
	public boolean start() {
		HikariConfig config;
		if(StrKit.notBlank(this.hikariConfigFile)){
			config = new HikariConfig(PropKit.use(hikariConfigFile).getProperties());
			Prop conf=PropKit.use(this.configFile);
			if(this.hasProxyDriver){
				this.proxyDriverClass=conf.get("jdbc.driverProxyClassName");
				this.driverClass=conf.get("jdbc.driver");
				config.setDriverClassName(proxyDriverClass);
				config.addDataSourceProperty("driver",driverClass);
			}else{
				this.driverClass=conf.get("jdbc.driver");
		...
		if(StrKit.notBlank(driverClass)&&StrKit.isBlank(proxyDriverClass)){
			config.setDriverClassName(driverClass);
		}
		if(StrKit.notBlank(proxyDriverClass)&&StrKit.notBlank(driverClass)){
			config.setDriverClassName(proxyDriverClass);
			config.addDataSourceProperty("driver",driverClass);
		}
